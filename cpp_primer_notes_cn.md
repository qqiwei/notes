...
对`static`变量来说，C将其作用域限定在定义它的文件中，另一个文件中的`extern`声明无法获取其定义。C++新标准消除了这个限制，并用未命名的命名空间来实现原属性。`namespace {}`中声明的对象即为未命名的命名空间，也就是不带命名空间的名字。全局命名空间可以用`::g_Name`来引用，但未命名的命名空间不能加如何作用域限定符，未命名的命名空间的对象作用域就是其所在作用域，虽然其生命周期同`static`变量一样。
...
泛型算法库中的有很多容易实现的函数，比如`fill` `fill_n` `equal` `accumulate`（`numeric`库）`for_each` `copy` `replace` `replace_copy` `replace_copy_if` `find_if` `unique`等。这些库函数会极大地提高编程效率，比如，可以用算法库中`partation`来快速实现leetcode 283或快排算法。

1. 部分修改容器元素的库函数，默认不修改容器的容量，可 能会需要用到`back_inserter(container)`指示做`push_back`操作。注：插入迭代器的行为不正常。此外，泛型迭代器还有IO迭代器，可用来从流读取/写入特定类型的元素。
