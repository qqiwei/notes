...
4 容器与继承：面向对象特性要依靠指针和引用这件事儿确实很不直接，同时，还导致另一个问题，就是容器中存储在继承体系中的一些对象的时候，要用共享指针。
