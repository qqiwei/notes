...
3. 为了避免预处理头文件引入的时候引入多次声明（或导致循环引用），C/C++支持头文件保护符`#ifndef`、`#define`和`#endif`，从而使变量和函数只被声明一次。类定义与函数声明放在头文件中，普通函数和类的成员函数的定义放在源文件中。定义所在的文件中也可引入其声明文件，以利用编译器检查声明和定义的一致性。
