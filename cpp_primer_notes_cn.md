...
建议：如果不改动入参，就要尽量声明出来。即，用常量类型作函数入参，以及，用引用代替指针。

**引用**的本质是没有空指针引用的问题的顶层常量指针，顶层常量就是不指向别的对象的指针。C原生的数组名很像是引用，但作为参数的数组类型将被当作一个非常量指针。

指针的**常量限定**分两种：顶层和底层（顶层常量限定的`const`写在解引用符的右边：`*const`），分别表示不指向其他地址（类似引用）和不改变所指向地址处的内容。`constexpr`是一种可以计算的字面值，它只要求编译器在编译期能算出值就行。另外，它对指针类型进行的是顶层常量限定，要同时限定底层常量的写法是：`constexpr const int *p`。

变量持常量引用时，因不改变所指向的对象这一属性而可以持有字面值。同时，变量持常量引用数组类型时，其维度是必要的。考虑非类型模板参数其实就是一个常量表达式的事实，而模板类型则同常量表达式类似地都在**编译期**可知。

#### 型别推导、`sizeof`

建议：用`using `（代替过去的`typedef`）来重命名类型。多用`auto`和`decltype`定类型，少人为写死类型。

型别推导是单纯的语法糖，目的是通过自动化获取类型来减轻开发的负担，并增加扩展性，它的规则有：
...
`sizeof(Type)`返回类型大小，`sizeof object`返回对象的类型大小。`string`、`vector`对象的类型将是固定的，其所持有的数据段是动态分配的。数组是个例外，`sizeof array_obj`不会将数组看作指针，而是会计算数组的大小。
...
3. 为了避免预处理头文件引入的时候引入多次声明，C/C++支持头文件保护符`#ifndef`、`#define`和`#endif`，从而使变量和函数只被声明一次。声明与定义都只能有一次。

对象声明在头文件中，定义在另一个文件中，调用点则通过`include`（只是在预处理的时候把被导入的文件拷贝到`include`的位置）引入包含声明的头文件。例外的是类，作为一种类型，一般定义在同名头文件中。
...
名字有作用域，对象（C++世界中的对象，一般具有超过面向对象编程概念里的对象的更宽泛的指涉，即，它指向程序中所有具体而非模板或类型的数据，地址数据也是数据）有生命周期。声明的是名字，定义的是对象，声明和定义在同一处的叫初始化（但其实没必要区分对象的定义、初始化和构造）。
...
对`static`变量来说，C将其作用域限定在定义它的文件中，另一个文件中的`extern`声明无法获取其定义。C++新标准消除了这个限制，并用未命名的命名空间来实现原属性。命名空间是一种作用域。全局命名空间可以用`::g_Name`来引用，未命名的命名空间则不加任何作用域限定符。

#### 编译选项、预处理变量

断言`assert(true_expr)`是一种预处理宏。可以通过定义预处理变量`NDEBUG`来关闭断言，编译选项也可以重置该变量。此外，C++也有一些别的由预处理器定义的魔术变量：`__FILE__`、`__LINE__`、`__FUNC__`和`__TIME__`等可用来打日志。
...
泛型算法库中的有很多容易实现的函数，比如`fill` `fill_n` `equal` `accumulate`（`numeric`库）`for_each` `copy` `replace` `replace_copy` `replace_copy_if` `find_if` `unique`等。这些库函数会极大地提高编程效率，比如，可以用算法库中`partation`来快速实现leetcode 283或快排算法。其中，有些可能需要对容器进行插入操作，可以使用`back_inserter(container)`、`front_inserter(container)`与`inserter(container, container.itr)`获取可调用`push_back`或`push`的迭代器对象。链表类有自己的 `merge`, `remove`, `reverse`, `sort` 和 `unique` 等库函数。
