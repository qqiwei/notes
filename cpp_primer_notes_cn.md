...
如果不改动入参，就要尽量声明出来。即，用常量类型作函数入参，以及，用引用代替指针。这也是接口设计所要求的，比如，`const &`类型可以接收一个非`const`或不支持值拷贝的对象，但反过来就不行了。变量是`const &`限定时，因不改变所指向的对象这一属性，使其可引用（持有）一个字面值（注意`const`或引用限定与**字面值**的细微差别）。数组名是一个指针（地址）类型的字面值，而作为形参的数组类型将被看作是在声明一个指针类型的局部变量。但`sizeof array_obj`不会将数组对象看作指针类型，而是会去计算数组占的内存大小。另外，变量是`const &`限定的数组类型时，其维度是必要的。可以参考泛型的设计原则：一般将模板参数类型限定为`const &`类型，且要求非类型模板参数必须是常量表达式。因为模板和常量表达式都需要在编译期间计算出真正要运行的类、函数与字面值。

#### `sizeof`、型别推导

`sizeof(Type)`、`sizeof object`返回该类型的对象占用的内存大小。类的大小是固定的，泛型类也能在编译器就将模板参数确定下来。像`string`、`vector`这样的可变容器的大小也是固定的，只是其所持有的数据段是动态分配的内存。这与Java中万物都是类对象，并分配到堆上，而栈上只保留基本类型的局部变量与对象引用的空间有所不同。
...
3. 为了避免预处理头文件引入的时候引入多次声明（或导致循环引用），C/C++支持头文件保护符`#ifndef`、`#define`和`#endif`，从而使变量和函数只被声明一次。类定义与函数声明放在头文件中，普通函数和类的成员函数的定义（类模板除外）放在源文件中。定义所在的文件中也可引入其声明文件，以利用编译器检查声明和定义的一致性。

**作用域、生命周期与`static`**

名字有作用域，对象（C++世界中的对象，一般具有超过面向对象编程概念里的对象的更宽泛的指涉，即，它意指程序中所有具体而非模板的数据，指针地址、函数指针也都算是数据、对象）有生命周期。声明的是名字，定义的是对象，声明和定义在同一处的叫初始化（但对象的定义、初始化和构造的内涵是一致的）。名字的作用域是程序文本的一部分，影响编译期间名字在程序中的可见性。对象的生命周期体现在程序执行过程中对象在内存中的存留时间。
...
函数模板可以经由编译器推断出参数类型，但类模板必须提供模板实参。与普通的类相比，类模板有更严格的限制：类模板成员函数的定义，要与类模板的定义一起放在头文件中。友元声明前要有模板友元的声明。这些限制可以总结成一句话，就是在模板定义的作用域内，除了模板参数之外的任何成分应该是定义明确的。这种明确限制保证了模板的灵活性。

包括成员函数在内，不被调用的函数不会被实例化，因而一些不满足类模板全部要求的类型也可以作为类模板的类型实参。类模板定义与类模板成员函数体在内（包括函数参数在内），属于类模板的作用域，内部可以省略类型参数，但函数体外（包括返回值类型处）需要类模板和类型参数的声明。

模板为友元与类型别名（用`typedef`只能重命名一个具体的类，但`using`可以重命名模板）提供了灵活性极大的支持。

作用域符号默认获取类模板的成员，要获取类模板的成员类型需要附加`typename`关键字限定。
